...
def isPointInShape(point, shapePoints):
    #check if inside the shape
    numOfIntersections = 0
    horizonLine = (point, (point[0] + screenWidth * 2, point[1]))

    for i in range(len(shapePoints)):
        edge = [shapePoints[i], shapePoints[(i+1)% len(shapePoints)]]

        if linenItersection(edge[0], edge[1], horizonLine[0], horizonLine[1], True) != None:
            numOfIntersections += 1

    #count inside polygon intersection method thingy
    if numOfIntersections % 2 != 0:
        return True
    return False

#spacial partitioning grid helper function
...
bounceDamping = 0.98            #energy loss on bounce with shape edge or screen edge
...
while running:
    ...
    #Not recommended in use when the target fps is not consistently being reached
    ...
    #reload edges (add shape edges to all edges) Note: this reloadedges is not the same as shape.reloadedges ... shape reload edges reloads local edge positions after movements or rotations
    edges = []
    for shape in shapes:
